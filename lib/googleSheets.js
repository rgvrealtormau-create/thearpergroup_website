...
// Preferred path: GOOGLE_PRIVATE_KEY_B64 holds the key as one base64 blob
// (base64 of the raw PEM text) — no newlines or quotes for a dashboard
// textarea to mangle, so paste-truncation can't happen. Generate it with:
//   node -e "console.log(Buffer.from(require('./key.json').private_key).toString('base64'))"
// or, from the raw private_key string directly:
...
function resolvePrivateKey() {
  ...
  if (b64) {
    ...
    if (!decoded.includes('-----BEGIN')) {
      throw new Error('GOOGLE_PRIVATE_KEY_B64 did not decode to a valid PEM key');
    }
...
async function getGoogleAccessToken() {
  ...
  if (!email || !key) throw new Error('Missing GOOGLE_SERVICE_ACCOUNT_EMAIL / GOOGLE_PRIVATE_KEY(_B64)');
